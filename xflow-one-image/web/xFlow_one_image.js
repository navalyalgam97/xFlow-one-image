...
let _subgraphExportRequested = false;
let _originalGraphBackup = null;

function loadWorkflowOntoCanvasInstance(prompt, targetGraph, targetCanvas) {
  // Clear graph
  targetGraph.clear();

  const cleanId = (idStr) => {
    const num = parseInt(String(idStr).replace("FK:", "").replace("FKI:", ""));
  ...
  for (const id in prompt) {
    const pNode = prompt[id];
    const classType = pNode.class_type;

    const node = LiteGraph.createNode(classType);
    if (!node) {
      console.warn(`Could not create node of type: ${classType}`);
      continue;
    }

    const targetId = cleanId(id);
    node.id = targetId;
    node.title = (pNode._meta && pNode._meta.title) || classType;

    if (pNode.inputs) {
      for (const inputName in pNode.inputs) {
        const val = pNode.inputs[inputName];
        if (!Array.isArray(val)) {
          const widget = (node.widgets || []).find(w => w.name === inputName);
          if (widget) {
            widget.value = val;
          } else {
            node.setProperty(inputName, val);
          }
        }
      }
    }
  ...
  for (const id in prompt) {
    const pNode = prompt[id];
    const targetId = cleanId(id);
    const node = idMap[targetId];
    if (!node || !pNode.inputs) continue;

    for (const inputName in pNode.inputs) {
      const val = pNode.inputs[inputName];
      if (Array.isArray(val)) {
        const srcNodeId = cleanId(val[0]);
        const srcSlotIndex = val[1];
        const srcNode = idMap[srcNodeId];

        if (srcNode) {
          const inputIndex = node.findInputSlot(inputName);
          if (inputIndex !== -1) {
            srcNode.connect(srcSlotIndex, node, inputIndex);
          }
        }
      }
    }
  ...
  // 3. Topological Layout
  const levels = {};
  const visited = new Set();

  function getLevel(n) {
    if (!n) return 0;
    if (visited.has(n.id)) return levels[n.id] || 0;
    visited.add(n.id);
    ...
    if (n.inputs) {
      n.inputs.forEach(input => {
        if (input.link != null) {
          const link = targetGraph.links[input.link];
          if (link) {
            const parentNode = targetGraph.getNodeById(link.origin_id);
            if (parentNode) {
              maxParentLevel = Math.max(maxParentLevel, getLevel(parentNode));
            }
          }
        }
      });
  ...
  targetGraph._nodes.forEach(n => getLevel(n));

  const columns = {};
  targetGraph._nodes.forEach(n => {
  ...
  Object.keys(columns).sort((a, b) => a - b).forEach(colLvl => {
    const colNodes = columns[colLvl];
    const x = startX + colLvl * colWidth;
    let currentY = startY;
    colNodes.forEach(n => {
      n.pos = [x, currentY];
      const nodeHeight = Math.max(120, 50 + Math.max(n.inputs ? n.inputs.length : 0, n.outputs ? n.outputs.length : 0) * 22 + (n.widgets ? n.widgets.length : 0) * 32);
      currentY += nodeHeight + 50;
    });
  });

  targetCanvas.draw(true, true);
  if (targetGraph._nodes.length > 0) {
    // Zoom and position graph nicely
    targetCanvas.ds.offset = [50, 50];
    targetCanvas.ds.scale = 0.75;
  }
}
...
function DD(items, selected, onChange) {
  ...
  const trigTxt = mk("span", {
    fontSize: "11px", color: C.text, overflow: "hidden",
...
function Pill(txt, active, onClick) {
  const b = mk("button", {
    background: active ? "linear-gradient(135deg, #a3e635, #00ff66)" : C.bg2, color: active ? "#000" : C.text,
...
function ImgSlot(optional, onFile, onDims) {
  ...
  tx(lbl, "Add image");
  if (optional) {
    const optPill = mk("div", {
      fontSize: "6px", color: C.muted, letterSpacing: ".06em", fontWeight: "700",
...
(() => {
  ...
  api.addEventListener("execution_success", async (evt) => {
    ...
    try {
      const r = await api.fetchApi(`/flux_klein/gallery?offset=0&limit=20&subfolder=xFlow`);
  ...
  async beforeRegisterNodeDef(nodeType, nodeData) {
    ...
    nodeType.prototype._buildUI = function () {
      ...
      if (!document.getElementById("fk-styles")) {
        ...
        styleEl.textContent = `
          @import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&family=Outfit:wght@300;400;500;600;700;800&family=Share+Tech+Mono&display=swap');
          
          .fk-root, .fk-root button, .fk-root input, .fk-root select, .fk-root textarea {
            font-family: 'Outfit', 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;
          }
          
          @keyframes fk-firefly-glow {
            0% {
              box-shadow: -2px 0 4px -0.5px rgba(163, 230, 53, 0.3), 2px 0 4px -0.5px rgba(0, 255, 102, 0.3);
              filter: brightness(1);
            }
            50% {
              box-shadow: -7px 0 15px -1.5px rgba(163, 230, 53, 0.75), 7px 0 15px -1.5px rgba(0, 255, 102, 0.75), 0 0 25px 5px rgba(0, 255, 102, 0.2);
          ...
          .fk-gen-btn {
            background: linear-gradient(135deg, #a3e635, #00ff66) !important;
            color: #000 !important;
            transition: all .2s ease-in-out !important;
          }
          .fk-gen-btn:hover:not(:disabled) {
            box-shadow: -4px 0 8px -1.5px rgba(163, 230, 53, 0.45), 4px 0 8px -1.5px rgba(0, 255, 102, 0.45) !important;
            filter: brightness(1.03);
          }
          
          @keyframes fk-gallery-glow {
            0% {
              box-shadow: -1.5px 0 4px -0.5px rgba(163, 230, 53, 0.1), 1.5px 0 4px -0.5px rgba(0, 255, 102, 0.1);
              filter: brightness(1);
            }
            50% {
              box-shadow: -4px 0 10px -1px rgba(163, 230, 53, 0.18), 4px 0 10px -1px rgba(0, 255, 102, 0.18), 0 0 15px 3px rgba(0, 255, 102, 0.08);
          ...
          .fk-gallery-btn {
            background: linear-gradient(135deg, #a3e635, #00ff66) !important;
            color: #000 !important;
            animation: fk-gallery-glow 2s ease-in-out infinite !important;
            transition: all .2s ease-in-out !important;
          }
          .fk-gallery-btn:hover {
            filter: brightness(1.05) !important;
            box-shadow: -5px 0 12px -1px rgba(163, 230, 53, 0.25), 5px 0 12px -1px rgba(0, 255, 102, 0.25) !important;
          }
      ...
      const root = mk("div", {
        width: "100%", background: C.bg0, boxSizing: "border-box",
      ...
      scrollEl.addEventListener("wheel", e => {
        ...
        if (_fwdCv) _fwdCv.dispatchEvent(new WheelEvent("wheel", {
      ...
      const settHdr = mk("div", {
        display: "flex", alignItems: "center", justifyContent: "space-between",
        marginBottom: "16px", flexShrink: "0"
      });
      const settTitle = mk("div", {
        fontSize: "13px", fontWeight: "700", letterSpacing: ".06em",
        textTransform: "uppercase", color: C.text
      }); tx(settTitle, "Settings");
      const settClose = mk("button", {
        background: "transparent", border: `1px solid #e05555`,
      ...
      const modelF = mkModDD("Model", "/models/diffusion_models", S.model, v => { S.model = v; if (typeof _kvUpdateNote === "function") _kvUpdateNote(); }, "klein,zimage,turbo,krea");
      const teF = mkModDD("Text Encoder", "/models/text_encoders", S.textEncoder, v => S.textEncoder = v, "qwen,qwen3");
      const vaeF = mkModDD("VAE", "/models/vae", S.vae, v => S.vae = v, "flux2,qwen");
      ...
      const _turboNote = mk("div", { fontSize: "9px", color: "#f0a040", marginTop: "0px", marginBottom: "4px", display: "none" });
      tx(_turboNote, "⚡ Turbo model detected. Recommended: 8 steps, CFG 1-2, euler sampler.");
      ...
      const _kvUpdateNote = () => {
        ...
        _turboNote.style.display = (name.includes("turbo") || name.includes("zimage") || name.includes("krea")) ? "block" : "none";
        ...
        } else if (name.includes("turbo") || name.includes("zimage") || name.includes("krea")) {
          if (S.steps === 20 || S.steps === 4) { S.steps = 8; stepsInp.setVal(8); }
          if (S.cfg === 5 || S.cfg === 1) { S.cfg = 1.5; cfgInp.setVal(1.5); }
        } else {
      ...
      settingsOverlay.append(settHdr, modGrid, _kvNote, _turboNote, _baseNote, _loraBox, prefTitle, soundToggle.el, advUIToggle.el, extLoadersToggle.el, _dsRow);
      ...
      const galleryBtn = mk("button", {
        background: "linear-gradient(135deg, #a3e635, #00ff66)",
        border: "1.5px solid transparent",
        borderRadius: "6px", padding: "4px 11px", cursor: "pointer", color: "#000",
        fontSize: "11px", fontWeight: "700", display: "flex", alignItems: "center", gap: "5px",
        transition: "all .2s ease-in-out", outline: "none",
      });
      galleryBtn.classList.add("fk-gallery-btn");
      ...
      const _mkModelLink = (name, url, folderKey) => {
        const btn = mk("button", {
          fontSize: "10px", color: C.text, background: C.bg3, border: `1px solid ${C.border}`,
          borderRadius: "5px", padding: "3px 9px", cursor: "pointer", display: "inline-flex",
          alignItems: "center", gap: "4px", transition: "all .15s", flexShrink: "0", outline: "none"
        });
        ...
        svg.appendChild(p);
        const span = document.createElement("span"); span.textContent = name; btn.append(svg, span);
        btn.addEventListener("mouseenter", () => { if (!btn.disabled) { btn.style.borderColor = LIME; btn.style.color = LIME; } });
        btn.addEventListener("mouseleave", () => { if (!btn.disabled) { btn.style.borderColor = C.border; btn.style.color = C.text; } });

        let filename = name;
        if (!filename.endsWith(".safetensors") && !filename.endsWith(".onnx") && !filename.endsWith(".pt") && !filename.endsWith(".pth")) {
          try {
            const urlObj = new URL(url);
            const pathParts = urlObj.pathname.split("/");
            const fn = pathParts[pathParts.length - 1];
            if (fn && (fn.endsWith(".safetensors") || fn.endsWith(".onnx") || fn.endsWith(".pt") || fn.endsWith(".pth"))) {
              filename = fn;
            } else {
        ...
        const checkStatus = async () => {
          try {
            const r = await api.fetchApi(`/flux_klein/download_status?filename=${encodeURIComponent(filename)}&folder_key=${folderKey}`);
            if (r.ok) {
              ...
              if (res.status === "downloading") {
                ...
                span.textContent = `Cancel (${res.progress}%)`;
                ...
                span.textContent = `${name} (Downloaded)`;
                ...
                span.textContent = "Error: Click to Retry";
              } else {
                isDownloading = false;
                if (pollInt) { clearInterval(pollInt); pollInt = null; }
                btn.disabled = false;
        ...
        btn.onclick = async () => {
          if (isDownloading) {
            const confirmCancel = confirm(`Cancel downloading ${name}?`);
            if (!confirmCancel) return;
            btn.disabled = true;
            span.textContent = "Cancelling...";
            try {
              const r = await api.fetchApi("/flux_klein/cancel_download", {
                method: "POST",
          ...
          try {
            const r = await api.fetchApi("/flux_klein/download", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ url, folder_key: folderKey, filename })
            });
            if (r.ok) {
              if (!pollInt) pollInt = setInterval(checkStatus, 2000);
              checkStatus();
            } else {
              ...
              alert("Failed to start download: " + (err.error || "unknown error"));
            }
          } catch (e) {
            btn.disabled = false;
            btn.style.borderColor = "#ff4444";
      ...
      const _mkModelRow = (label, path, links, note) => {
        ...
        const folderKey = path.replace("models/", "").replace("/", "");
        links.forEach((l, i) => { if (i > 0) cw.appendChild(_mkOrSep()); cw.appendChild(_mkModelLink(l.name, l.url, folderKey)); });
      ...
      // Tab Headers
      const tabContainer = mk("div", { display: "flex", gap: "8px", marginBottom: "12px", borderBottom: `1px solid ${C.border}`, paddingBottom: "8px" });
      ...
      const tabKlein = mk("button", {});
      tx(tabKlein, "FLUX.2 Klein");
      const tabZImage = mk("button", {});
      tx(tabZImage, "Z-Image");
      const tabKrea = mk("button", {});
      tx(tabKrea, "Krea 2");
      tabContainer.append(tabKlein, tabZImage, tabKrea);

      // Lists
      const listKlein = mk("div", { display: "flex", flexDirection: "column", gap: "2px" });
      const listZImage = mk("div", { display: "flex", flexDirection: "column", gap: "2px" });
      const listKrea = mk("div", { display: "flex", flexDirection: "column", gap: "2px" });
      ...
      listZImage.append(
        _mkModelRow("Diffusion Model", "models/diffusion_models/", [
          { name: "Z-Image Turbo (6B)", url: "https://huggingface.co/Comfy-Org/z_image_turbo/resolve/main/split_files/diffusion_models/z_image_turbo_bf16.safetensors" },
        ]),
        _mkModelRow("Text Encoder", "models/text_encoders/", [
          { name: "qwen_3_4b", url: "https://huggingface.co/Comfy-Org/vae-text-encorder-for-flux-klein-4b/resolve/main/split_files/text_encoders/qwen_3_4b.safetensors" },
          { name: "qwen_3_4b_fp4", url: "https://huggingface.co/Comfy-Org/vae-text-encorder-for-flux-klein-4b/resolve/main/split_files/text_encoders/qwen_3_4b_fp4_flux2.safetensors" },
        ], "For Z-Image models (uses standard Flux 4b text encoder)"),
        _mkModelRow("VAE", "models/vae/", [
          { name: "flux2-vae", url: "https://huggingface.co/Comfy-Org/vae-text-encorder-for-flux-klein-9b/resolve/main/split_files/vae/flux2-vae.safetensors" },
        ], "Uses standard Flux 2 VAE")
      );

      // Populate Krea 2
      listKrea.append(
        _mkModelRow("Diffusion Model", "models/diffusion_models/", [
          { name: "Krea 2 Turbo (12B)", url: "https://huggingface.co/krea/Krea-2-Turbo/resolve/main/turbo.safetensors" },
          { name: "Krea 2 RAW (12B)", url: "https://huggingface.co/krea/Krea-2-Raw/resolve/main/raw.safetensors" },
        ]),
        _mkModelRow("Text Encoder (Qwen3-VL)", "models/text_encoders/", [
          { name: "qwen3_vl (Krea 2)", url: "https://huggingface.co/krea/Krea-2-Raw/resolve/main/text_encoder/model.safetensors" },
        ], "For Krea 2 models"),
        _mkModelRow("VAE", "models/vae/", [
          { name: "qwen_vae (Krea 2)", url: "https://huggingface.co/krea/Krea-2-Raw/resolve/main/vae/diffusion_pytorch_model.safetensors" },
        ])
      );

      const setTab = (active) => {
        [tabKlein, tabZImage, tabKrea].forEach(t => Object.assign(t.style, inactiveTabStyle));
        [listKlein, listZImage, listKrea].forEach(l => l.style.display = "none");
        if (active === "klein") {
          Object.assign(tabKlein.style, activeTabStyle);
          listKlein.style.display = "flex";
        } else if (active === "zimage") {
          Object.assign(tabZImage.style, activeTabStyle);
          listZImage.style.display = "flex";
        } else if (active === "krea") {
          Object.assign(tabKrea.style, activeTabStyle);
          listKrea.style.display = "flex";
        }
      };

      tabKlein.onclick = () => setTab("klein");
      tabZImage.onclick = () => setTab("zimage");
      tabKrea.onclick = () => setTab("krea");
      setTab("klein");

      const modelsList = mk("div", { display: "flex", flexDirection: "column", gap: "2px", marginBottom: "8px" });
      modelsList.append(tabContainer, listKlein, listZImage, listKrea);
      ...
      [
        ...
        { name: "🌐 Z-Image", url: "https://github.com/Tongyi-MAI/Z-Image" },
      ...
      [
        { name: "⚙ GitHub", url: "https://github.com/navalyalgam97", color: C.text },
      ...
      // ── RETURN TO CUSTOM NODE BUTTON ─────────────────────────────────────
      const returnBtn = mk("button", {
        position: "fixed",
        bottom: "20px",
        right: "20px",
        background: "#0f0f1b",
      ...
      tx(returnBtn, "✓ Save & Return to Custom Node (xFlow)");
      returnBtn.onmouseenter = () => {
        returnBtn.style.background = LIME;
        returnBtn.style.color = "#000";
      };
      returnBtn.onmouseleave = () => {
        returnBtn.style.background = "#0f0f1b";
        returnBtn.style.color = LIME;
      };

      const syncSettingsFromSubgraphMainGraph = (graphInstance) => {
        if (!graphInstance) return;

        const getWidgetValue = (node, name) => {
        ...
        graphInstance._nodes.forEach(n => {
          const classType = n.comfyClass || n.type;

          // 1. Sampler settings (agnostic to custom sampler class like ClownsharkSampler)
          const steps = getWidgetValue(n, "steps");
          if (steps !== null) {
            S.steps = Number(steps);
            hasAdvancedChanges = true;
            if (typeof stepsInp !== "undefined") stepsInp.setVal(steps);
          }

          const cfg = getWidgetValue(n, "cfg");
          if (cfg !== null) {
            S.cfg = Number(cfg);
            hasAdvancedChanges = true;
            if (typeof cfgInp !== "undefined") cfgInp.setVal(cfg);
          }

          const sampler = getWidgetValue(n, "sampler_name");
          if (sampler !== null) {
            S.sampler = sampler;
            hasAdvancedChanges = true;
            if (typeof samplerDD !== "undefined" && samplerDD.set) samplerDD.set(sampler);
          }

          const scheduler = getWidgetValue(n, "scheduler");
          if (scheduler !== null) {
            S.scheduler = scheduler;
            hasAdvancedChanges = true;
            if (typeof schedulerDD !== "undefined" && schedulerDD.set) schedulerDD.set(scheduler);
          }

          const seed = getWidgetValue(n, "seed");
          if (seed !== null) {
            S.seed = Number(seed);
            if (typeof seedInp !== "undefined") seedInp.setVal(seed);
            if (typeof _advSeedInp !== "undefined") _advSeedInp.setVal(seed);
          }

          // 2. Positive prompt
          if (classType === "CLIPTextEncode" || (n.title || "").toLowerCase().includes("positive")) {
            const txt = getWidgetValue(n, "text");
            if (txt !== null && !(n.title || "").toLowerCase().includes("negative")) {
              S.prompt = txt;
              if (typeof _promptTARef !== "undefined" && _promptTARef) _promptTARef.value = txt;
            }
          }

          // 3. Models
          const unet = getWidgetValue(n, "unet_name");
          if (unet !== null) { S.model = unet; if (typeof modelF !== "undefined" && modelF.dd) modelF.dd.set(unet); }

          const clip = getWidgetValue(n, "clip_name");
          if (clip !== null) { S.textEncoder = clip; if (typeof teF !== "undefined" && teF.dd) teF.dd.set(clip); }

          const vae = getWidgetValue(n, "vae_name");
          if (vae !== null) { S.vae = vae; if (typeof vaeF !== "undefined" && vaeF.dd) vaeF.dd.set(vae); }
        });

        if (hasAdvancedChanges) {
          S.advancedUI = true;
          if (typeof _advRefresh === "function") {
            _advRefresh();
          }
        }
      ...
      returnBtn.onclick = () => {
        try {
          console.log("[FluxKlein] Return button clicked. Syncing settings...");
          syncSettingsFromSubgraphMainGraph(app.graph);

          // Clear node cache so it reconstructs cleanly from the new localStorage state
          const nodeId = _liveId();
          if (window.__fluxklein_nodes && window.__fluxklein_nodes[nodeId]) {
            delete window.__fluxklein_nodes[nodeId];
          }

          if (_originalGraphBackup) {
            console.log("[FluxKlein] Restoring original graph...");
            app.graph.configure(_originalGraphBackup);
            _originalGraphBackup = null;
          } else {
            console.warn("[FluxKlein] No original graph backup found!");
          }
        } catch (e) {
          console.error("[FluxKlein] Error in returnBtn.onclick:", e);
        } finally {
      ...
      const subgraphBtn = mk("button", {
        background: "transparent", border: `1.5px solid ${C.borderH}`,
        borderRadius: "6px", padding: "4px 8px",
        cursor: "pointer", color: C.muted,
        display: "flex", alignItems: "center", justifyContent: "center",
        transition: "opacity .15s, border-color .15s, color .15s", outline: "none"
      }, { title: "Convert to Node-based Workflow" });
      subgraphBtn.innerHTML = `<svg viewBox="0 0 24 24" width="11" height="11" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="9"/><rect x="14" y="12" width="7" height="9"/><path d="M10 7.5h4v9h-4"/></svg>`;
      subgraphBtn.onmouseenter = () => { subgraphBtn.style.borderColor = LIME; subgraphBtn.style.color = LIME; };
      subgraphBtn.onmouseleave = () => { subgraphBtn.style.borderColor = C.borderH; subgraphBtn.style.color = C.muted; };
      subgraphBtn.onclick = () => {
        _originalGraphBackup = app.graph.serialize();
        _subgraphExportRequested = true;
        genBtn.click();
      };
      topBarRight.append(galleryBtn, tipsBtn, settingsBtn, subgraphBtn, layoutBtn, fsNodeBtn);
      ...
      function setPill(p) {
        ...
        // Tab presets auto-selection
        if (p === "t2i") {
          // T2I: Auto-select Z-Image models & settings
          if (typeof modelF !== "undefined" && modelF.dd) {
            const list = modelF.dd.items || [];
            const match = list.find(o => o.toLowerCase().includes("z_image_turbo_bf16")) ||
                          list.find(o => o.toLowerCase().includes("z_image") || o.toLowerCase().includes("zimage"));
            if (match) { S.model = match; modelF.dd.set(match); }
          }
          if (typeof teF !== "undefined" && teF.dd) {
            const list = teF.dd.items || [];
            const match = list.find(o => o.toLowerCase().includes("qwen_3_4b")) ||
                          list.find(o => o.toLowerCase().includes("qwen3_4b")) ||
                          list.find(o => o.toLowerCase().includes("qwen"));
            if (match) { S.textEncoder = match; teF.dd.set(match); }
          }
          if (typeof vaeF !== "undefined" && vaeF.dd) {
            const list = vaeF.dd.items || [];
            const match = list.find(o => o.toLowerCase().includes("ae.safetensors") || o.toLowerCase().includes("ae")) ||
                          list.find(o => o.toLowerCase().includes("flux2-vae") || o.toLowerCase().includes("flux2"));
            if (match) { S.vae = match; vaeF.dd.set(match); }
          }
          ...
          if (typeof stepsInp !== "undefined") stepsInp.setVal(8);
          if (typeof cfgInp !== "undefined") cfgInp.setVal(1.5);
          if (typeof samplerDD !== "undefined" && samplerDD.set) samplerDD.set("euler");
          if (typeof schedulerDD !== "undefined" && schedulerDD.set) schedulerDD.set("simple");

        } else if (p === "inpaint" || p === "edit") {
          // PAINT (inpaint) & EDIT: Auto-select specific models and preferences
          if (typeof modelF !== "undefined" && modelF.dd) {
            const list = modelF.dd.items || [];
            const match = list.find(o => o.toLowerCase().includes("flux-2-klein-9b-fp8")) ||
              list.find(o => o.toLowerCase().includes("flux-2-klein") || o.toLowerCase().includes("klein"));
            if (match) { S.model = match; modelF.dd.set(match); }
          }
          if (typeof teF !== "undefined" && teF.dd) {
            const list = teF.dd.items || [];
            const match = list.find(o => o.toLowerCase().includes("qwen_3_8b_fp4mixed")) ||
              list.find(o => o.toLowerCase().includes("qwen_3_8b") || o.toLowerCase().includes("qwen3_8b")) ||
              list.find(o => o.toLowerCase().includes("qwen"));
            if (match) { S.textEncoder = match; teF.dd.set(match); }
          }
          if (typeof vaeF !== "undefined" && vaeF.dd) {
            const list = vaeF.dd.items || [];
            const match = list.find(o => o.toLowerCase().includes("flux2-vae")) ||
              list.find(o => o.toLowerCase().includes("flux2")) ||
              list.find(o => o.toLowerCase().includes("vae"));
            if (match) { S.vae = match; vaeF.dd.set(match); }
          }
          if (typeof fsLoraF !== "undefined" && fsLoraF.dd) {
            const list = fsLoraF.dd.items || [];
            const match = list.find(o => o.toLowerCase().includes("bfs_head_v1_flux-klein_4b")) ||
              list.find(o => o.toLowerCase().includes("bfs_head")) ||
              list.find(o => o.toLowerCase().includes("bfs"));
            if (match) { S.fsLora = match; fsLoraF.dd.set(match); }
          }
          if (typeof poseLoraF !== "undefined" && poseLoraF.dd) {
            const list = poseLoraF.dd.items || [];
            const match = list.find(o => o.toLowerCase().includes("refcontrol_v2_poses")) ||
              list.find(o => o.toLowerCase().includes("refcontrol")) ||
              list.find(o => o.toLowerCase().includes("pose"));
            if (match) { S.poseLora = match === "none" ? "" : match; poseLoraF.dd.set(match); }
          }
          if (typeof bgF !== "undefined" && bgF.dd) {
            S.bgRemovalModel = "";
            bgF.dd.set("none");
          }

          // Preferences:
          // 1. Notification sound: enabled
          soundEnabled = true;
          if (typeof soundToggle !== "undefined" && soundToggle._setChecked) {
            soundToggle._setChecked(true);
          }

          // 2. Advanced control: enabled
          S.advancedUI = true;
          if (typeof advUIToggle !== "undefined" && advUIToggle._setChecked) {
            advUIToggle._setChecked(true);
          }
          if (typeof _advRefresh === "function") _advRefresh();

          // 3. External model/clip/vae inputs: disabled
          S.extLoaders = false;
          if (typeof extLoadersToggle !== "undefined" && extLoadersToggle._setChecked) {
            extLoadersToggle._setChecked(false);
          }
          if (typeof _applyExtLoaders === "function") _applyExtLoaders(false);

          // 4. Downscale reference images: disabled
          S.downscaleRef = false;
          if (typeof _dsTrack !== "undefined" && typeof _dsThumb !== "undefined") {
            _dsTrack.style.background = C.dim;
            _dsThumb.style.left = "2px";
            _dsThumb.style.background = "#888";
          }
          if (typeof _dsApplyEnabled === "function") _dsApplyEnabled();

          // Steps/CFG/Sampler/Scheduler: standard Flux 2 Klein settings
          S.steps = 4;
          S.cfg = 1.0;
          S.sampler = "er_sde";
          S.scheduler = "simple";

          if (typeof stepsInp !== "undefined") stepsInp.setVal(4);
          if (typeof cfgInp !== "undefined") cfgInp.setVal(1.0);
          if (typeof samplerDD !== "undefined" && samplerDD.set) samplerDD.set("er_sde");
          if (typeof schedulerDD !== "undefined" && schedulerDD.set) schedulerDD.set("simple");

        } else {
          // Others: Auto-select Flux 2 Klein models & settings
          if (typeof modelF !== "undefined" && modelF.dd) {
            const list = modelF.dd.items || [];
            const match = list.find(o => o.toLowerCase().includes("flux-2-klein") || o.toLowerCase().includes("klein"));
            if (match) { S.model = match; modelF.dd.set(match); }
          }
          if (typeof teF !== "undefined" && teF.dd) {
            const list = teF.dd.items || [];
            const match = list.find(o => o.toLowerCase().includes("qwen_3_8b") || o.toLowerCase().includes("qwen3_8b"));
            if (match) { S.textEncoder = match; teF.dd.set(match); }
          }
          if (typeof vaeF !== "undefined" && vaeF.dd) {
            const list = vaeF.dd.items || [];
            const match = list.find(o => o.toLowerCase().includes("flux2-vae") || o.toLowerCase().includes("flux2"));
            if (match) { S.vae = match; vaeF.dd.set(match); }
          }
        ...
        [pillT2I, pillI2I, pillEdit, pillInpaint, pillFaceswap, pillPose].forEach(b => {
          const isActive =
            ...
            (b === pillPose && p === "pose");
          b.style.background = isActive ? "linear-gradient(135deg, #a3e635, #00ff66)" : C.bg2;
          b.style.color = isActive ? "#000" : C.text;
          b.style.borderColor = isActive ? "transparent" : C.border;
      ...
      const leftPanel = mk("div", {
        ...
        width: "330px", flexShrink: "0", minHeight: "0", overflowY: "auto", overflowX: "hidden",
        padding: "0 15px 15px 15px", boxSizing: "border-box",
      });
        ...
        ov._open = (type, src, name, opts) => {
          ...
          if (type === "image") {
            ...
            const cLine = mk("div", {
              ...
              background: LIME, left: "calc(100% - 1px)", boxShadow: "0 0 8px rgba(0,255,102,.5)",
              display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", zIndex: "4"
            });
            const cHandle = mk("div", {
      ...
      const _i2iDims = (() => {
        ...
        const el = mk("div", {
          ...
          background: "rgba(0,255,102,.13)", borderColor: "rgba(0,255,102,.5)",
        });
      ...
      const _i2iApplyState = () => {
        ...
        if (_i2iUseOrigSize) {
          ...
          _i2iDims.style.background = "rgba(0,255,102,.13)";
          _i2iDims.style.borderColor = "rgba(0,255,102,.5)";
      ...
      i2iSlider.addEventListener("mousedown", (e) => e.stopPropagation());
      i2iSlider.addEventListener("pointerdown", (e) => e.stopPropagation());
      ...
      const _mkDimsLbl = (slotKey) => {
        ...
        const _refresh = () => {
          ...
          if (_w && _h) {
            ...
            if (active) {
              el.style.color = LIME;
              el.style.background = "rgba(0,255,102,.13)";
              el.style.border = `1px solid rgba(0,255,102,.5)`;
              el.style.boxShadow = "0 0 0 1px rgba(0,255,102,.12)";
            } else {
      ...
      const _paintDimsLbl = mk("div", {
        ...
        borderRadius: "5px", padding: "2px 6px", boxSizing: "border-box",
        border: `1px solid rgba(0,255,102,.4)`, background: "rgba(0,255,102,.08)", color: LIME,
      });
      ...
      const _setPaintMode = (mode) => {
        ...
        _sketchBtn.style.background = sketchActive ? "rgba(0,255,102,.12)" : C.bg2;
        ...
        _inpaintBtn.style.background = inpaintActive ? "rgba(0,255,102,.12)" : C.bg2;
      ...
      const _mkSkTool = (label, title) => {
        ...
        const lbl = mk("div", {
          fontSize: "7px", fontWeight: "700", letterSpacing: ".04em",
        ...
        b._setActive = (on) => {
          b._active = on;
          b.style.background = on ? "rgba(0,255,102,.15)" : "transparent";
      ...
      const _sketchSizeGroup = mk("div", {
        ...
        border: `1.5px solid rgba(0,255,102,.35)`, borderRadius: "7px",
        padding: "3px 8px", background: "rgba(0,255,102,.04)",
      });
      const _skSzLabel = mk("div", {
        fontSize: "8px", fontWeight: "700", color: LIME, letterSpacing: ".07em",
      ...
      _skArLockBtn.onclick = () => {
        ...
        _skArLockBtn.style.background = _skArLocked ? "rgba(0,255,102,.08)" : "transparent";
      ...
      const _sketchResApplyBtn = mk("button", {
        background: "rgba(0,255,102,.15)", border: `1px solid ${LIME}`, borderRadius: "4px",
      ...
      const _sketchSetSizeApplied = (applied) => {
        ...
        _sketchResApplyBtn.style.background = applied ? "rgba(255,255,255,.08)" : "rgba(0,255,102,.15)";
      ...
      _sketchResApplyBtn.onmouseenter = () => {
        ...
        else _sketchResApplyBtn.style.background = "rgba(0,255,102,.28)";
      };
      _sketchResApplyBtn.onmouseleave = () => {
        ...
        else _sketchResApplyBtn.style.background = "rgba(0,255,102,.15)";
      };
      ...
      const _mkZBtn = (t, icon) => {
        const b = mk("button", {
          background: C.bg3, border: `1px solid ${C.borderH}`, borderRadius: "4px",
      ...
      const _sketchZoomReset = mk("button", {
        background: C.bg3, border: `1px solid ${C.borderH}`, borderRadius: "4px",
      ...
      const _sketchCloseBtn = mk("button", {
      ...
      const _mkSliderRow = (lbl, min, max, init, onSet) => {
        ...
        const lblEl = mk("div", {
          fontSize: "7px", color: C.muted, letterSpacing: ".06em",
          textTransform: "uppercase", marginBottom: "1px", gridColumn: "1/-1"
        });
        tx(lblEl, lbl);
        const slider = mk("input", {
          width: "100%", cursor: "pointer", accentColor: LIME,
      ...
      const _mkGrpLbl = (t) => {
        const d = mk("div", {
          fontSize: "7px", fontWeight: "700", color: C.muted,
      ...
      const _skPh1 = mk("div", {
        fontSize: "13px", fontWeight: "700", color: "rgba(255,255,255,.35)",
      ...
      const _sketchLayersPTitle = mk("div", {
        fontSize: "8px", fontWeight: "700", color: C.muted,
      ...
      const _skRemoveBgRun = async () => {
        ...
        try {
          ...
          const preR = await api.fetchApi("/flux_klein/gallery?offset=0&limit=200&subfolder=xFlow/assets");
          ...
          const postR = await api.fetchApi("/flux_klein/gallery?offset=0&limit=200&subfolder=xFlow/assets");
      ...
      const _mkAddMenuItem = (label, icon, fn) => {
        ...
        item.append(ico, lbl);
        item.onmouseenter = () => { item.style.background = "rgba(0,255,102,.10)"; item.style.color = LIME; ico.style.color = LIME; };
      ...
      const _sketchRebuildLayerUI = () => {
        ...
        _sketchLayers.forEach((layer, i) => {
          ...
          const row = mk("div", {
            ...
            background: isSelected ? "rgba(240,180,40,.13)" : (isActive ? "rgba(0,255,102,.08)" : "transparent"),
          ...
          dupRowBtn.onclick = (e) => {
            ...
            const dup = {
              canvas: c, ctx: c.getContext("2d", { willReadFrequently: true }), maskCanvas: mc2, maskCtx: mctx2,
      ...
      const _maskCanvasWrap = mk("div", { position: "relative", transformOrigin: "0 0", cursor: "inherit", zIndex: "1" });
      _maskCanvasWrap.appendChild(_maskDisplayCanvas);

      // Cursor circle (follows mouse inside viewport only, not double-scaled by canvas zoom)
      const _maskCursor = mk("div", {
        position: "absolute", borderRadius: "50%",
        border: "2px solid #00ff66",
        boxShadow: "0 0 0 1px #000",
        pointerEvents: "none", display: "none", zIndex: "99999", boxSizing: "border-box",
        transform: "translate(-50%,-50%)",
      });

      // Marquee (rectangle select) preview — inside the wrap so it scales with zoom
      const _maskRectPreview = mk("div", {
        position: "absolute", border: `2px dashed ${LIME}`, background: "rgba(0,255,102,.18)",
      ...
      _maskViewport.appendChild(_maskCursor);
      ...
      const _mkMaskToolBtn = (icon, label) => {
        ...
        b._setActive = (on) => {
          b._on = on;
          b.style.background = on ? "rgba(0,255,102,.15)" : C.bg2;
          b.style.borderColor = on ? LIME : C.border;
      ...
      _maskSizeSlider.addEventListener("mousedown", (e) => e.stopPropagation());
      _maskSizeSlider.addEventListener("pointerdown", (e) => e.stopPropagation());
      ...
      _maskHardSlider.addEventListener("mousedown", (e) => e.stopPropagation());
      _maskHardSlider.addEventListener("pointerdown", (e) => e.stopPropagation());
      ...
      const _mkOpField = (label) => {
        ...
        const lbl = mk("div", {
          fontSize: "7px", fontWeight: "600", letterSpacing: ".06em",
      ...
      const _opApplyBadgeState = () => {
        ...
        if (_opUseExpandedSize) {
          ...
          _opDimsLbl.style.background = hasExpand ? "rgba(0,255,102,.1)" : C.bg3;
          _opDimsLbl.style.borderColor = hasExpand ? "rgba(0,255,102,.4)" : C.borderH;
      ...
      _opFeatherSlider.addEventListener("mousedown", (e) => e.stopPropagation());
      _opFeatherSlider.addEventListener("pointerdown", (e) => e.stopPropagation());
      ...
      const _inpDimsLbl = mk("div", {
        ...
        border: `1px solid rgba(0,255,102,.5)`, background: "rgba(0,255,102,.1)", color: LIME,
        transition: "color .15s,background .15s,border-color .15s",
      });
      ...
      _inpFeatherSlider.addEventListener("mousedown", (e) => e.stopPropagation());
      _inpFeatherSlider.addEventListener("pointerdown", (e) => e.stopPropagation());
      ...
      const _inpApplyBadgeState = () => {
        if (_inpUseOrigSize) {
          ...
          _inpDimsLbl.style.background = "rgba(0,255,102,.1)";
          _inpDimsLbl.style.borderColor = "rgba(0,255,102,.5)";
      ...
      _maskViewport.addEventListener("pointerdown", (e) => {
        if (e.pointerType === "mouse" && e.button === 1 || (e.button === 0 && e.altKey) || (e.button === 0 && _maskSpaceDown)) {
          e.preventDefault();
          _maskPanning = true;
        ...
        try {
          _maskViewport.setPointerCapture(e.pointerId);
        } catch (err) {}
      ...
      _maskViewport.addEventListener("pointermove", (e) => {
        if (_maskOv.style.display === "none") return;
        if (_maskPanning) {
        ...
        const { scaleX, scaleY, r } = _maskVpScale();
        const inside = e.clientX >= r.left && e.clientY >= r.top && e.clientX <= r.right && e.clientY <= r.bottom;
        
        if (inside || _maskDrawing) {
          const pos = _maskCanvasCoords(e);
          const vx = (e.clientX - r.left) / scaleX;
          const vy = (e.clientY - r.top) / scaleY;
          _maskCursor.style.display = "block";
          _maskCursor.style.left = vx + "px";
          _maskCursor.style.top = vy + "px";
          _maskCursor.style.width = (_maskSize * _maskZoom) + "px";
          _maskCursor.style.height = (_maskSize * _maskZoom) + "px";
          if (_maskDrawing) {
      ...
      _maskViewport.addEventListener("pointerup", (e) => {
        if (_maskPanning) {
          _maskPanning = false;
          _maskViewport.style.cursor = _maskSpaceDown ? "grab" : (_maskMode === "inpaint" ? (_maskTool === "rect" ? "crosshair" : "none") : "default");
        }
        if (_maskDrawing && _maskTool === "rect") _maskCommitRect();
        _maskDrawing = false;
        try {
          _maskViewport.releasePointerCapture(e.pointerId);
        } catch (err) {}
      });

      _maskViewport.addEventListener("pointerleave", () => {
        if (!_maskDrawing) {
          _maskCursor.style.display = "none";
        }
      });
      ...
      const _arSetLocked = (locked) => {
        ...
        _arLockBtn.style.background = locked ? "rgba(0,255,102,.08)" : "transparent";
      ...
      const genRow = mk("div", { display: "flex", gap: "0", alignItems: "stretch", marginTop: "0", width: "100%", boxSizing: "border-box" });
      const genBtn = mk("button", {
        background: LIME, color: "#000", border: "2px solid transparent", borderRadius: "8px",
      ...
      tx(genBtn, "Generate");
      genBtn.classList.add("fk-gen-btn");
      ...
      _batchBtn.classList.add("fk-gen-btn");
      ...
      [1, 2, 3, 4].forEach(n => {
        ...
        row.onmouseenter = () => { row.style.background = "rgba(0,255,102,.1)"; };
      ...
      const _fsApplyState = () => {
        ...
        if (_fsUseOrigSize) {
          ...
          _fsTargetDims.style.background = "rgba(0,255,102,.13)";
          _fsTargetDims.style.borderColor = "rgba(0,255,102,.5)";
      ...
      function _poseApplyBadges() {
        ...
        if (pd.w && pd.h) {
          ...
          if (active) {
            _poseImgDims.style.color = LIME; _poseImgDims.style.background = "rgba(0,255,102,.13)";
            _poseImgDims.style.borderColor = "rgba(0,255,102,.5)";
      ...
      _poseImgDims.onclick = () => {
        const d = _poseImgDims._getDims(); if (!d.w || !d.h) return;
      ...
      const _timerRow = mk("div", {
        display: "flex", alignItems: "center", justifyContent: "space-between",
        background: "#080808", border: `1px solid ${C.border}`, borderRadius: "6px",
        padding: "5px 12px", boxSizing: "border-box", width: "100%", marginTop: "auto",
        marginBottom: "7px"
      });
      const _timerLbl = mk("div", {
        fontSize: "10px", color: C.muted, textTransform: "uppercase", letterSpacing: "0.05em",
        fontFamily: "'Share Tech Mono', monospace"
      });
      tx(_timerLbl, "Gen Time");
      const _timerVal = mk("div", {
        fontSize: "13px", color: "#00ff66", fontWeight: "700",
        textShadow: "0 0 6px rgba(0,255,102,0.45)", letterSpacing: "0.02em",
        fontFamily: "'Share Tech Mono', monospace"
      });
      tx(_timerVal, "0.0s");
      _timerRow.append(_timerLbl, _timerVal);

      leftPanel.append(i2iPanel, editPanel, inpaintPanel, faceswapPanel, posePanel, resSect, advPanel, seedRow, _seedLockedWarn, _timerRow, genRow);
      ...
      const previewBox = mk("div", {
        ...
        background: "#090909 url('data:image/svg+xml;base64,') repeat",
      ...
      comparerBase.addEventListener("dragstart", e => e.preventDefault());
      ...
      comparerGenImg.addEventListener("dragstart", e => e.preventDefault());
      ...
      const comparerLine = mk("div", {
        ...
        boxShadow: "0 0 8px rgba(0,255,102,.5)",
        display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
        gap: "6px", zIndex: "4", pointerEvents: "none",
      });
      ...
      comparerWrap.addEventListener("pointerdown", e => {
        _cmpDragging = true;
        e.preventDefault();
        try {
          comparerWrap.setPointerCapture(e.pointerId);
        } catch (err) {}
        const r = comparerWrap.getBoundingClientRect();
        _cmpSetPct((e.clientX - r.left) / r.width * 100);
      });
      comparerWrap.addEventListener("pointermove", e => {
        if (!_cmpDragging) return;
        const r = comparerWrap.getBoundingClientRect();
        _cmpSetPct((e.clientX - r.left) / r.width * 100);
      });
      comparerWrap.addEventListener("pointerup", e => {
        _cmpDragging = false;
        try {
          comparerWrap.releasePointerCapture(e.pointerId);
        } catch (err) {}
      });
      comparerWrap.addEventListener("pointercancel", () => {
        _cmpDragging = false;
      });
      ...
      const _confirmTxt = mk("div", {
        fontSize: "11px", fontWeight: "600", color: "rgba(255,200,200,.9)",
      ...
      const previewSaveBtn = mk("button", {
        ...
        background: "rgba(0,255,102,.9)", border: "1px solid rgba(0,255,102,.5)",
      ...
      const _asTrack = mk("div", {
        width: "26px", height: "14px", borderRadius: "7px", position: "relative",
        transition: "background .18s", flexShrink: "0"
      });
      const _asThumb = mk("div", {
      ...
      const _asLbl = mk("span", {
        fontSize: "9px", fontWeight: "700", color: "rgba(255,255,255,.85)",
      ...
      const _asApply = () => {
        ...
        _asTrack.style.background = on ? "rgba(0,255,102,.85)" : "rgba(255,255,255,.14)";
      ...
      const _ulPSub = mk("div", { width: "100%", height: "1px", background: "rgba(0,255,102,.10)", marginTop: "-6px" });
      const _ulRows = mk("div", {
      ...
      const _mkULRow = (idx) => {
        ...
        const _rowNum = mk("span", {
          ...
          background: "rgba(0,255,102,.1)", color: LIME, flexShrink: "0",
        });
        ...
        const enTog = mk("div", {
          ...
          background: _enInit ? "rgba(0,255,102,.85)" : "rgba(255,255,255,.13)",
        });
        enTog.title = "Toggle this LoRA on/off (keeps it loaded when off)";
        const enThumb = mk("div", {
          position: "absolute", top: "2px", left: _enInit ? "16px" : "2px",
        ...
        const trigLbl = mk("div", {
          fontSize: "8px", fontWeight: "700", color: C.muted, whiteSpace: "nowrap",
          flexShrink: "0", letterSpacing: ".06em", textTransform: "uppercase", opacity: ".8"
        });
        tx(trigLbl, "Trigger");
        const trigVal = mk("div", {
          fontSize: "10px", color: LIME, flex: "1", minWidth: "0",
        ...
        tx(trigSaveBtn, "Save");
        trigSaveBtn.onmouseenter = () => { trigSaveBtn.style.background = "rgba(0,255,102,.12)"; trigSaveBtn.style.borderColor = LIME; trigSaveBtn.style.color = LIME; };
        ...
        ulStr.onblur = () => {
          const p = _pf(ulStr.value); S.userLoras[idx].strength = isNaN(p) ? 1 : p;
        ...
        const _applyEnabled = () => {
          ...
          enTog.style.background = en ? "rgba(0,255,102,.85)" : "rgba(255,255,255,.13)";
      ...
      _ulAddBtn = mk("button", {
        ...
        background: "rgba(0,255,102,.06)", border: `1px dashed rgba(0,255,102,.3)`,
      ...
      _ulAddBtn.onmouseenter = () => { _ulAddBtn.style.background = "rgba(0,255,102,.12)"; _ulAddBtn.style.borderColor = LIME; };
      _ulAddBtn.onmouseleave = () => { _ulAddBtn.style.background = "rgba(0,255,102,.06)"; _ulAddBtn.style.borderColor = "rgba(0,255,102,.3)"; };
      ...
      const _ulBtn = mk("button", {
        background: "linear-gradient(135deg,rgba(0,255,102,.10),rgba(0,255,102,.04))",
        border: "1.5px solid rgba(0,255,102,.35)", cursor: "pointer",
        ...
        boxShadow: "0 0 0 0 rgba(0,255,102,0)",
      });
      ...
      _ulBtn.onmouseenter = () => { _ulBtn.style.background = "linear-gradient(135deg,rgba(0,255,102,.18),rgba(0,255,102,.08))"; _ulBtn.style.borderColor = LIME; _ulBtn.style.boxShadow = "0 0 8px rgba(0,255,102,.12)"; };
      _ulBtn.onmouseleave = () => { _ulBtn.style.background = "linear-gradient(135deg,rgba(0,255,102,.10),rgba(0,255,102,.04))"; _ulBtn.style.borderColor = "rgba(0,255,102,.35)"; _ulBtn.style.boxShadow = "0 0 0 0 rgba(0,255,102,0)"; };
      ...
      const _ulUpdateBtn = () => {
        ...
        _ulBtn.style.borderColor = n > 0 ? LIME : "rgba(0,255,102,.35)";
      ...
      // Header (static)
      const _inspireHdr = mk("div", {
        display: "flex", alignItems: "center", justifyContent: "space-between",
        marginBottom: "10px", flexShrink: "0"
      });
      const _inspireTitleEl = mk("div", {
      ...
      const _inspireCloseBtn = mk("button", {
        background: "none", border: "none", cursor: "pointer",
        ...
        if (def.categories) {
          ...
          if (activePill === "t2i") {
            ...
            const tmplForm = mk("div", {
              ...
              border: `1px solid rgba(0,255,102,.3)`, borderRadius: "16px",
              padding: "10px 12px", background: "rgba(0,255,102,.04)", boxSizing: "border-box",
            });
            ...
            const _renderTmplPills = () => {
              ...
              _templates.forEach((t, idx) => {
                ...
                tx(pill, t.name);
                pill.onmouseenter = () => { if (_tmplOpenIdx !== idx) { pill.style.background = "rgba(0,255,102,.10)"; pill.style.borderColor = LIME; pill.style.color = LIME; } };
              ...
              const tmplAddPill = mk("button", {
                ...
                border: `1px dashed rgba(0,255,102,.4)`,
                background: "rgba(0,255,102,.05)", color: "rgba(0,255,102,.6)",
                outline: "none", transition: "all .15s", flexShrink: "0",
              });
              tx(tmplAddPill, "+ Add"); tmplAddPill.dataset.addpill = "1";
              tmplAddPill.onmouseenter = () => { tmplAddPill.style.borderColor = LIME; tmplAddPill.style.background = "rgba(0,255,102,.12)"; tmplAddPill.style.color = LIME; };
              tmplAddPill.onmouseleave = () => { tmplAddPill.style.borderColor = "rgba(0,255,102,.4)"; tmplAddPill.style.background = "rgba(0,255,102,.05)"; tmplAddPill.style.color = "rgba(0,255,102,.6)"; };
              tmplAddPill.onclick = () => {
          ...
          def.categories.forEach(({ cat, items }) => {
            ...
            items.forEach(({ label, prompt, dual }, itemIdx) => {
              ...
              if (dual) {
                ...
                tx(pill, label);
                pill.onmouseenter = () => { if (_openDetailIdx !== itemIdx) { pill.style.background = "rgba(0,255,102,.10)"; pill.style.borderColor = LIME; pill.style.color = LIME; } };
            ...
            tx(addPill, "+ Add");
            addPill.onmouseenter = () => { addPill.style.borderColor = LIME; addPill.style.background = "rgba(0,255,102,.12)"; addPill.style.color = LIME; };
            addPill.onmouseleave = () => { addPill.style.borderColor = "rgba(0,255,102,.4)"; addPill.style.background = "rgba(0,255,102,.05)"; addPill.style.color = "rgba(0,255,102,.6)"; };
        ...
        if (showTabs) {
          tabs.forEach((_tab, i) => {
            ...
            const tab = mk("button", {
              ...
              background: i === 0 ? "rgba(0,255,102,.08)" : "transparent",
            });
            tx(tab, cat);
            tab.onclick = () => {
              tabRow.querySelectorAll("button").forEach((t, j) => {
                ...
                t.style.background = j === i ? "rgba(0,255,102,.08)" : "transparent";
              });
      ...
      const errMain = mk("div", {
        background: "linear-gradient(180deg,rgba(255,103,103,.12),rgba(255,103,103,.05))",
      ...
      const _startTimer = () => {
        ...
        _timerVal.style.color = "#00ff66";
        _timerVal.style.textShadow = "0 0 6px rgba(0,255,102,0.45)";
        _timerInterval = setInterval(() => {
          const elapsed = (Date.now() - _timerStartTime) / 1000;
          tx(_timerVal, elapsed.toFixed(1) + "s");
        }, 100);
      };
      const _stopTimer = () => {
        if (_timerInterval) {
      ...
      const _resetGenBtn = () => {
        ...
        genBtn.style.animation = "none"; genBtn.style.color = "#000";
      ...
      genBtn.onclick = async () => {
        ...
        S._pendingMeta = {
          ...
          ...(S.advancedUI ? {
            steps: S.steps || 4, cfg: S.cfg !== undefined ? S.cfg : 1,
        ...
        genBtn.style.animation = "fk-firefly-glow 1.5s ease-in-out infinite";
        genBtn.style.color = "#000"; genBtn.style.border = "2px solid transparent";
        previewDelBtn.style.display = "none";
        ...
        try {
          const prevR = await api.fetchApi("/flux_klein/gallery?offset=0&limit=200&subfolder=xFlow");
        ...
        if (isInpaintMode) {
          ...
          set(WFI.save, "filename_prefix", "xFlow/xFlow");
          ...
          set(WFO.save, "filename_prefix", "xFlow/xFlow");
          ...
          set(WFF.save, "filename_prefix", "xFlow/xFlow");
          ...
          set(WFP.save, "filename_prefix", "xFlow/xFlow");
          ...
          set("FK:86", "filename_prefix", "xFlow/xFlow");
          ...
          set(WF.saveImage, "filename_prefix", "xFlow/xFlow");
        ...
        if (typeof _subgraphExportRequested !== "undefined" && _subgraphExportRequested) {
          _subgraphExportRequested = false;
          resetBtn();
          loadWorkflowOntoCanvasInstance(prompt, app.graph, app.canvas);
      ...
      const _mkGalCtxItem = (label, icon, onClick) => {
        ...
        row.append(ico, lbl);
        row.onmouseenter = () => { row.style.background = "rgba(0,255,102,.10)"; row.style.color = LIME; ico.style.color = LIME; };
      ...
      const _mkGalCtxSec = (label) => {
        const h = mk("div", {
          padding: "6px 12px 3px", fontSize: "8px", fontWeight: "700",
      ...
      const _setGalFavBtn = (active) => {
        _galFavOnly = active;
        galFavBtn.style.background = active ? `rgba(0,255,102,.18)` : "transparent";
        ...
        galleryOverlay.style.background = active ?
          "linear-gradient(180deg,rgba(0,255,102,.06) 0%,rgba(0,255,102,.02) 40%,rgba(0,0,0,0) 100%), #0a0a0a" :
          "#0a0a0a";
      };
      ...
      const lbFilename = mk("div", {
        fontSize: "11px", color: C.muted, flex: "1",
        overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap"
      });
      const lbClose = mk("button", {
        background: "transparent", border: `1px solid #e05555`,
      ...
      const lbMeta = mk("div", {
        ...
        background: "linear-gradient(180deg,rgba(0,255,102,.07),rgba(0,255,102,.02))",
        border: "1px solid rgba(0,255,102,.2)", borderRadius: "10px",
      ...
      const _lbChip = (label, val) => {
        ...
        const cl = mk("div", {
          fontSize: "8px", color: C.muted, fontWeight: "700",
          letterSpacing: ".07em", textTransform: "uppercase"
        });
        tx(cl, label);
      ...
      const lbImgThumb = mk("img", {
        height: "40px", borderRadius: "4px", objectFit: "cover",
        display: "none", alignSelf: "center", border: `1px solid ${C.border}`
      });
      const lbImgThumb2 = mk("img", {
        height: "40px", borderRadius: "4px", objectFit: "cover",
        display: "none", alignSelf: "center", border: `1px solid ${C.border}`
      });
      ...
      const lbFavBtn = mk("button", {
        background: "rgba(20,20,30,.85)", border: "1px solid rgba(0,255,102,.2)",
        borderRadius: "6px", width: "40px", height: "40px", flexShrink: "0",
        cursor: "pointer", outline: "none",
        transition: "background .2s,border-color .2s,color .2s",
        display: "flex", alignItems: "center", justifyContent: "center",
        color: "rgba(0,255,102,.35)", alignSelf: "stretch"
      });
      const _lbFavApplyStyle = (hover) => {
        if (_lbFavActive) {
          lbFavBtn.style.background = hover ? "rgba(0,255,102,.22)" : "rgba(0,255,102,.15)";
          lbFavBtn.style.borderColor = hover ? LIME : LIME;
          lbFavBtn.style.color = hover ? "#fff" : LIME;
          lbFavBtn.style.boxShadow = hover ? "0 0 10px rgba(0,255,102,.3)" : "0 0 6px rgba(0,255,102,.15)";
        } else {
          lbFavBtn.style.background = hover ? "rgba(0,255,102,.08)" : "rgba(20,20,30,.85)";
          lbFavBtn.style.borderColor = hover ? "rgba(0,255,102,.5)" : "rgba(0,255,102,.2)";
          lbFavBtn.style.color = hover ? "rgba(0,255,102,.85)" : "rgba(0,255,102,.35)";
      ...
      }); // marginLeft:auto pushes THIS + lbOpenBtn to the right edge
      const _lbOutSvg = (() => {
      ...
      const _setLbFav = (isFav) => {
        ...
        lightbox.style.background = isFav ?
          "linear-gradient(180deg,rgba(0,255,102,.18) 0%,rgba(0,255,102,.06) 50%,rgba(0,0,0,0) 100%), #0a0a0a" :
          "#0a0a0a";
      };
      ...
      const _lbApplyMeta = async (meta) => {
        ...
        try {
          ...
          if (meta.w && meta.h && (pill === "t2i" || pill === "edit")) {
            ...
            if (preset) {
              S.resLabel = preset.label; S.resW = preset.w; S.resH = preset.h; S.isCustomRes = false;
              resDD.set(preset.label); customResRow.style.display = "none";
            }
            else {
              S.isCustomRes = true; S.customW = meta.w; S.customH = meta.h; S.resLabel = "Custom…";
      ...
      const _galAppend = (images, startIdx) => {
        images.forEach((v, i) => {
          ...
          if (v.favorite === true) {
            ...
            cell.style.background = "linear-gradient(180deg,rgba(0,255,102,.12) 0%,rgba(0,255,102,.04) 100%)";
            cell.style.borderColor = "rgba(0,255,102,.4)";
          }
      ...
      const galLoad = async (reset = true, limit = GAL_LIMIT) => {
        ...
        try {
          if (_galFavOnly) {
            ...
            const r = await api.fetchApi(`/flux_klein/gallery?offset=${_galOffset}&limit=${limit}&subfolder=xFlow&favonly=1`);
            const d = await r.json();
            ...
            const r = await api.fetchApi(`/flux_klein/gallery?offset=${_galOffset}&limit=${limit}&subfolder=xFlow`);
            const d = await r.json();
            const newImgs = d.images || [];
            _galTotal = d.total || 0;
      ...
      root.addEventListener("pointerdown", (e) => {
        ...
        const fwd = (type, ev, ptr) => {
          ...
          const opts = {
            button: 1, buttons: (type === "mouseup" || type === "pointerup") ? 0 : 4,
        ...
        const up = (ev) => {
          if (!_fkPanning) return; _fkPanning = false;
      ...
      const _loadModels = () => api.fetchApi("/flux_klein/models")
        ...
        .then(d => {
          ...
          if (modelList.length) { const v = pick(modelList, S.model, "zimage"); S.model = v; modelF.dd.updateItems(modelList); modelF.dd.set(v); }
          ...
          if (vaeList.length) { const v = pick(vaeList, S.vae, "ae"); S.vae = v; vaeF.dd.updateItems(vaeList); vaeF.dd.set(v); }
